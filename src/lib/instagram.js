(function (exports, Logger) {
  var InstagramLibrary = function (options) {
    ...
    var parseAuthenticationToken = function () {
      ...
      if (accessToken) {
        Logger.log(
          'InstagramLibrary parseAuthenticationToken accessToken found',
        );
      } else {
        ...
        if (kv.length === 2 && kv[0] === '#access_token') {
          Logger.log(
            'InstagramLibrary parseAuthenticationToken accessToken found in url',
          );
          ...
          if (history && history.replaceState) {
            var replacement = document.location
              .toString()
          ...
          Logger.log(
            'InstagramLibrary parseAuthenticationToken accessToken not found',
          );
        }
      }
    };
  ...
  var InstagramPopular = function (options) {
    this.library = options.library;
  };
  InstagramPopular.prototype = new Instagram({ endpoint: 'media/popular' });

  var InstagramUser = function (options) {
    this.library = options.library;
  };
  InstagramUser.prototype = new Instagram({
    endpoint: 'users/self/media/recent',
  });

  var InstagramFeed = function (options) {
    this.library = options.library;
  };
  InstagramFeed.prototype = new Instagram({ endpoint: 'users/self/feed' });

  var InstagramLiked = function (options) {
    this.library = options.library;
  };
  InstagramLiked.prototype = new Instagram({
